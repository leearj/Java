...
public class Graph {
	...
	System.out.println("**********************************************");
	System.out.println("StartCityIndex: "+startCityIndex);
	System.out.println("current index: "+currentCityIndex);
	System.out.println(visitList);
	System.out.println("traceDist: "+traceDist);
	System.out.println("tracePath BEFORE: "+tracePath);
	System.out.println("bestDist BEFORE: "+bestDist);
	System.out.println("bestPath BEFORE:"+bestPath);
	System.out.println();
		...
		while (visitList.size() != 0) {
			...
			if(nodes.get(currentCityIndex-1).neighbor.size()!=0 && areUnvisited){
				...
				for (int i = 0; i < nodes.get(currentCityIndex - 1).neighbor.size(); i++){
					...
					if(bestDist.get(currentNeighborIndex-1) > (costToGetHere + currentNeighborDist)){
						System.out.println("bestDist.get(currentNeighborIndex-1: "+bestDist.get(currentNeighborIndex-1));
						System.out.println("costToGetHere + currentNeihborDist"+costToGetHere+currentNeighborDist);
					...
					System.out.println("currentNeighborDist: "+currentNeighborDist+ " with index: "+currentNeighborIndex );	
				}
			}
			System.out.println("costToGetHere[before]: "+ costToGetHere);
			System.out.println("bestPath size: "+bestPath.size());
			System.out.println("bestDist AFTER: "+bestDist);
			System.out.println("bestPath AFTER: "+bestPath);
			
			//Update it for next iteration.
			if(nextDist != Integer.MAX_VALUE && visitList.contains(nextIndex) && nextIndex != -1){
				System.out.println("nextDist is ~~~~: "+nextIndex+"with the dist:"+nextDist);
			...
			System.out.println("costToGetHere[nextIndex val added]: "+costToGetHere);
			System.out.println("### Neighbors were found in this node ###\n\n");
			Dijkstra(nextIndex);
			}
			...
			//else if there is no neighbors or nextIndex isn't in visitList
				...
				System.out.println("nxtIndex: "+nxtIndex);
				System.out.println("oh there is no neighbors or next index isn't in visitList!");
				System.out.println("I'll renew the path for the next!");
				...
				System.out.println("I'll go to: "+nxtIndex+",which has value:"+nxtIndexDist+"\n");
